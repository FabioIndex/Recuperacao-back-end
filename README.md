...
Inicie o SQLite:
sqlite3 sistema.db
Crie a tabela (Copie e cole este comando):
sql
...
Instale as ferramentas no terminal:
npm install express sqlite3
Ligue o servidor:
node app.js
Abra no navegador: Clique em "Open in Browser".
Verifique o banco: Após cadastrar, pare o servidor (Ctrl + C) e digite:
sqlite3 sistema.db "SELECT * FROM usuarios;"
Entrega de Recuperação (Classroom):
Para validar, poste no comentário da tarefa:
O nome e a cidade que você cadastrou.
O que aconteceu quando você rodou o comando SELECT no terminal? (Apareceu seu nome lá?).
